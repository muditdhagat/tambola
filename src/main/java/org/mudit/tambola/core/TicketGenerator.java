...
public class TicketGenerator {
  ...
  public static Integer GROUP_SEQ = 1;
  ...
  public List<TicketSet> generateTicketSets(int ticketPerSet, int totalticketSets) {
      
      int ticketSetCode = 100;
      
      List<TicketSet> ticketSets = new ArrayList<TicketSet>();
      
      if(ticketPerSet == 0) ticketPerSet = 1;

      List<Ticket> tickets = generateTickets(totalticketSets * ticketPerSet);
      
      for(int i = 0; i < totalticketSets; i++) {
	  TicketSet ticketSet = new TicketSet();
	  ticketSet.setTickets(tickets.subList(i, i + ticketPerSet));
	  // Generate 3-Digit Code
	  ticketSet.setTicketId("" + ticketSetCode++);
  ...
  private List<Ticket> createTickets(Map<Integer, Map<Integer, List<Integer>>> datasets) {
    ...
    String groupId = Integer.toHexString(GROUP_SEQ++);
  ...
  private void createDatasets(
      Map<Integer, List<Integer>> columnData, Map<Integer, Map<Integer, List<Integer>>> datasets) {
    ...
    for (int col = 0; col < 9; col++) {
      ...
      for (int setNum = 0; setNum < 6; setNum++) {
        ...
        int randomNumber =
            columnValues.remove(randomIndex); // Get and Remove the number from the column data
    ...
    int randomNumber =
        lastColumn.remove(randomIndex); // Get and Remove the number from the column data

    int randomSetIndex = TicketHelper.generateRandomNumber(0, datasets.size() - 1);
    datasets
        .get(randomSetIndex)
        .get(8)
        .add(randomNumber); // Get the dataset from the datasets using the random index and add the
    // number to the last column

    // Iterate over this process 4 times for all the remaining numbers
    // Keep the column limit to 2 for 3 passes and then 3 for the last 4th pass to ensure less 3
    // column numbers.
      ...
      for (int columnIndex = 0; columnIndex < 9; columnIndex++) {
        ...
        if (column.size() != 0) {
          ...
          while (!datasetFound && setIndex.size() != 0) {
            ...
            int randomCheckedDatasetsIndex =
                TicketHelper.generateRandomNumber(0, setIndex.size() - 1);
            randomSetIndex = setIndex.remove(randomCheckedDatasetsIndex);

            Map<Integer, List<Integer>> dataset = datasets.get(randomSetIndex);

            if (TicketHelper.getNumberOfElementsInDataset(dataset) < 15
                && dataset.get(columnIndex).size() < (it == 3 ? 3 : 2)) {
              datasetFound = true;
  ...
  public static void main(String[] args) {
    ...
    //				while(!columnFilled) { // Ideally at this point all the rows for the columns should be
    // empty.
    //					int number = columnValues.remove(0);
    //					int randomRowIndex = TicketHelper.generateRandomNumber(0, 1); // If can put this on
    // either 1/2 Row only since we have two
    //					if(Objects.isNull(ticket.getData()[randomRowIndex][columnIndex]) &&
    // ticket.getRowCount(randomRowIndex) < 5 ) {
    //						if(randomRowIndex == 0) {
    //
